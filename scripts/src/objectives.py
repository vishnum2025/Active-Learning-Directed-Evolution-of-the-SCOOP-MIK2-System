...
class Combo(Objective):
    ...
    def __init__(self, protein, encoding):
            # Detect if MODIFY-based dataset exists, otherwise fallback
        modify_path = f"data/{protein}/from_MODIFY_gb1.csv"
        if os.path.exists(modify_path):
            print(f"[INFO] Using MODIFY GB1 dataset: {modify_path}")
            fitness_df = pd.read_csv(modify_path)
            fitness_col = "modify_fitness" if "modify_fitness" in fitness_df.columns else "fitness"
        else:
            print(f"[INFO] Using default ALDE dataset for {protein}")
            fitness_df = pd.read_csv(f"data/{protein}/fitness.csv")
            fitness_col = "fitness"

        # Normalize fitness
        self.y = torch.tensor(fitness_df[fitness_col].values).double()
        self.y = self.y / self.y.max()

        # Detect correct one-hot encoding file
        onehot_path = f"data/{protein}/one_hot_encoding_modify.pt"
        if not os.path.exists(onehot_path):
            onehot_path = f"data/{protein}/{encoding}_x.pt"

        print(f"[INFO] Loading encoding from: {onehot_path}")
        self.X = torch.load(onehot_path)
